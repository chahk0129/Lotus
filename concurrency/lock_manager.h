...
class lock_manager_t: public cc_manager_t {
public:
    lock_manager_t(txn_t* txn);
    ~lock_manager_t() { }
    ...
    RC         get_row(uint32_t access_idx);
    RC         get_row(cc_manager_t::RowAccess* access);
    ...
    RC         process_commit();
    void       cleanup(RC rc);
    void       abort(uint32_t abort_access_idx);
    void       abort(cc_manager_t::RowAccess* access);
    void       clear();

    char*      get_data(uint64_t key, uint32_t table_id);
    void       serialize_commit(uint32_t node_id, char*& data, uint32_t& size);
    ...
    void       clear_nodes_involved() { _remote_nodes.clear(); }
...
private:
    ...
    void       commit_insdel();
    void       prefetch_row(char* ptr, uint32_t size);

    std::set<uint32_t>       _nodes_involved; // for commit
    std::set<uint32_t>       _remote_nodes;   // for txn requests
