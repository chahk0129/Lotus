...
#include "utils/latency_collector.h"
#include "utils/debug.h"
#include "utils/helper.h"
#include "index/onesided/dex/hash_table.h"
...
// containeer
static std::vector<std::vector<void *>> local_page_set;
thread_local static LatencyCollector decision;
// FIXME(BT): need a sensitive study for the following parameters
extern int probing_length;
extern int num_pages_to_sample;
#define CONCURRENT 1
extern uint64_t sample_num;
extern uint64_t total_sample_loop;
extern uint64_t wrong_page_state;
extern uint64_t wrong_iteration;
extern uint64_t wrong_parent;
extern uint64_t wrong_parent_lock;
extern uint64_t wrong_parent_check;
extern uint64_t wrong_evict;
extern uint64_t one_state;
extern uint64_t zero_state;
extern uint64_t three_state;
...
public:
  ...
  CacheManager(uint64_t cache_capacity, double cooling_ratio, double rpc_rate,
               double admission_rate, global_addr_t *root_ptr = nullptr) {
      debug::notify_info("rpc rate is %lf", rpc_rate);
    ...
    local_page_set.resize(g_num_client_threads);
  ...
  /***** Cache management *****/
  /*----------------------------------*/
  NodeBase *search_in_cache(global_addr_t global_node) {
  ...
  bool is_in_cache(global_addr_t global_node) {
    uint64_t node = global_node.val;
    if (node & swizzle_tag) {
      return true;
    }
  ...
  NodeBase *cache_insert(global_addr_t &global_node, NodeBase *cache_node,
    ...
    global_addr_t snapshot = global_node;
  ...
  void sample_page() {
    ...
    while (true) {
      ...
      if (cur_page == nullptr)
        continue;
      auto parent = reinterpret_cast<BTreeInner *>(cur_page->parent_ptr);
  ...
  void insert_local_set(uint64_t addr) {
    ...
    node->pos_state = 3;
    local_page_set[GET_THD_ID].push_back(reinterpret_cast<void *>(addr));
  }

  void *get_local_page_set() {
    if (local_page_set[GET_THD_ID].empty()) {
      return nullptr;
    }
    auto ret = local_page_set[GET_THD_ID].back();
    local_page_set[GET_THD_ID].pop_back();
    return ret;
  }
  ...
  void *try_get_empty_page() {
    ...
    while (true) {
	  ...
	  */
          ...
          if (page == nullptr && (!local_page_set[GET_THD_ID].empty())) {
            page = get_local_page_set();
            assert(page != nullptr);
          }
	  /*
    ...
    if (page != nullptr) {
	auto node = reinterpret_cast<NodeBase*>(page);
	if(!node->isLocked()) // hokeun
	    node->setLockState();
	if(node->pos_state == 2)
	    node->pos_state = 0;
  ...
  void swizzling(global_addr_t &global_addr, NodeBase *parent,
                 unsigned child_idx, NodeBase *child) {
    if (parent) {
      if (!check_parent_child_info(parent, child)) {
        std::cout << "Gloobal addr = " << global_addr.node_id << " " << global_addr.addr << std::endl;
  ...
  void unswizzling(global_addr_t &global_addr, NodeBase *parent,
                   unsigned child_idx, NodeBase *child) {
    auto inner_parent = reinterpret_cast<BTreeInner *>(parent);
  ...
  bool sync_or_not(BTreeInner *inner, uint64_t idx) {
    if (inner->level == 255)
      return true;
  ...
  void opportunistic_sample() {
    if (state == 1 && local_page_set[GET_THD_ID].empty()) {
  ...
  NodeBase *cache_get(global_addr_t node, NodeBase *parent, unsigned child_idx,
    ...
    if (target_page == nullptr) {
      ...
      cold_to_hot(node, reinterpret_cast<void **>(&return_page), parent,
                  child_idx, refresh);
      // bool sync_read =
      //     sync_or_not(reinterpret_cast<BTreeInner *>(parent),
      //     child_idx);
      // NodeBase *return_page = nullptr;
      // auto ret =
    ...
    bool exclusive_success =
        get_exclusive_node(global_addr_t(node), target_node);
    ...
    if (!new_check_limit_match(parent, target_node, child_idx)) {
      ...
      if (target_node->isShared()) {
        ...
        auto remote_target_node = reinterpret_cast<BTreeInner *>(
  ...
  int cold_to_hot_with_admission(global_addr_t global_node, void **ret_page,
                                 NodeBase *parent, unsigned child_idx,
                                 bool &refresh, Key k, Value &result,
                                 bool &success, message_t::type_t rpc_type) {
    ...
    if (state == 1 && idx >= admission_idx) {
      ...
      case message_t::type_t::REQUEST_DEX_LOOKUP: {
        ...
        auto cur_leaf = reinterpret_cast<BTreeLeaf<Value> *>(buffer_page);
  ...
  // 0 means cold to hot succeeds
  // 1 means one-sided update succeeds
  int cold_to_hot_with_admission_for_scan(global_addr_t global_node,
    ...
    if (idx >= admission_idx) {
      ...
      if (!cur_leaf->rangeValid(k)) {
        ...
        scan_num = cur_leaf->range_scan(k, scan_num, v_buffer);
  ...
  int cold_to_hot_with_rpc(global_addr_t global_node, void **ret_page,
                           NodeBase *parent, unsigned child_idx, bool &refresh,
                           Key k, Value &result, bool &success,
                           message_t::type_t rpc_type) {
    ...
    if (state == 1 && flag && rpc_rate_ != 0) {
      global_addr_t leaf_addr;
      int ret = 0;
      switch (rpc_type) {
      case message_t::type_t::REQUEST_DEX_LOOKUP:
        ret = rpc_lookup<Value>(global_node, k, result);
        break;

      case message_t::type_t::REQUEST_DEX_UPDATE:
        ret = rpc_update<Value>(global_node, k, result, leaf_addr);
        break;

      case message_t::type_t::REQUEST_DEX_INSERT:
        ret = rpc_insert<Value>(global_node, k, result, leaf_addr);
        break;

      case message_t::type_t::REQUEST_DEX_DELETE:
        ret = rpc_remove<Value>(global_node, k, leaf_addr);
        break;
      ...
      int ret_flag = 1;
      if (ret <= 0) {
	if(rpc_type == message_t::type_t::REQUEST_DEX_INSERT)
	  ret_flag = -1;
	else
	  success = false;
        //assert(rpc_type == message_t::type_t::REQUEST_DEX_INSERT);
        //ret_flag = -1;
      } else if (ret == 1) {
        success = true;
  ...
  // -1 means failure and retry
  // 0 means cold to hot succeeds
  int cold_to_hot(global_addr_t global_node, void **ret_page, NodeBase *parent,
                  unsigned child_idx, bool &refresh) {
    // No sync read is needed because leaf nodes are exclusive
    bool sync_read =
        sync_or_not(reinterpret_cast<BTreeInner *>(parent), child_idx);
    auto ret = simple_cold_to_hot(global_node, ret_page, parent, child_idx,
                                  refresh, sync_read);
    if (ret == 0) {
      void *old_flag = nullptr;
  ...
  bool get_exclusive_node(global_addr_t node, NodeBase *target_node) {
    if (target_node->pos_state == 1) {
      ...
      auto target_parent =
          reinterpret_cast<BTreeInner *>(target_node->parent_ptr);
  ...
  void replace_child(BTreeInner *parent, NodeBase *cur_node,
                     NodeBase *new_node) {
    int idx_in_parent = -1;
    if (parent->level == 255) {
      idx_in_parent = 0;
  ...
  // buffer
  void updatest_replace(NodeBase *cur_node, BTreeInner *parent,
                        NodeBase *remote_cur_node) {
    fill_local_page_set();
    global_addr_t remote_addr = cur_node->remote_address;
    ...
    if (target_node == nullptr) {
      ...
      // global_addr_t swizzling = remote_addr;
  ...
  bool remote_to_cache(void *cache_page, global_addr_t global_node, bool sync_read) {
  ...
  // -1 means failure and retry
  // 0 means succeeds
  int simple_cold_to_hot(global_addr_t node, void **ret_page, NodeBase *parent,
  ...
  void flush_all() {
    ...
    for (uint64_t i = 0; i < page_num; ++i) {
      ...
      if (cur_page->dirty) {
        ...
        memcpy((void*)page_buffer, cur_page, pageSize);
