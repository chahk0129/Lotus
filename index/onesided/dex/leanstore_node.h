...
template <class Payload> struct BTreeLeaf : public BTreeLeafBase {
  ...
  // BTreeLeaf<Payload> *next_leaf=nullptr;
  global_addr_t next_leaf = global_addr_t::null();

  uint64_t back_version;

  uint64_t dummy[2]; // pack for 1024 granularity

  BTreeLeaf(global_addr_t remote) {
  ...
  uint32_t range_scan(Key k, uint32_t num, Payload *&array) {
  ...
  // No use
  void populate(std::pair<Key, Payload> &array, int begin, int end) {
    count = end - begin;
    assert(count <= maxEntries);
  ...
  void split(Key &sep, BTreeLeaf *newLeaf, global_addr_t remote_node) {
    front_version++;
    is_smo = true;
...
struct BTreeInner : public BTreeInnerBase {
  static const uint64_t maxEntries =
      ...
      (sizeof(Key) + sizeof(global_addr_t));
  global_addr_t children[maxEntries];
  Key keys[maxEntries];
  uint64_t back_version;
  uint64_t dummy;

  BTreeInner(uint8_t cur_level, global_addr_t remote) {
    level = cur_level;
    remote_address = remote;
    parent_ptr = nullptr;
    count = 0;
  ...
  void split(Key &sep, BTreeInner *newInner) {
    ...
    memcpy(newInner->children, children + count + 1,
           sizeof(global_addr_t) * (newInner->count + 1));
