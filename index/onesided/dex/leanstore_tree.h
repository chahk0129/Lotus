...
#include "index/onesided/dex/leanstore_cache.h"
#include "index/onesided/dex/leanstore_wr.h"
...
#if PARTITIONED
namespace cachepush {
// Global variable declarations (definitions moved to .cpp file)
extern uint64_t push_down_counter;
extern uint64_t cache_miss_counter;
extern uint64_t admission_counter;
extern uint64_t cache_miss_array[8]; // Collect the cache miss in bottom 5 levels
// In the memory side, we only maintain a BTree without any statistics
// In the client, we maintain a LRU cache
extern thread_local uint64_t total_nanoseconds;
extern thread_local uint64_t total_sample_times;

// #define FINE_GRAINED 1

template <class Value> class BTree : public idx_wrapper_t<Value>{
public:
  global_addr_t root_ptr_ptr_;
  global_addr_t root_lock_ptr_; // To the global lock
  global_addr_t root;
  OptLock root_lock_; // FIXME(BT): should be removed later
  int tree_id_;
  BTreeInner *super_root_; // which can be
  ...
  CacheManager<Value> cache;

  BTree(int tree_id, uint64_t cache_mb, double sample_rate, double admission_rate)
      : tree_id_(tree_id),
        cache(cache_mb * 1024 * 1024 / pageSize, 0.1, sample_rate, admission_rate, &root) {
    std::cout << "start generating tree" << std::endl;
    super_root_ = new BTreeInner(255, global_addr_t::null());
    super_root_->pos_state = 2;
    // super_root_->shared = true;
    fetch_root_ptr(0);
    //root_lock_ptr_ = get_root_lock_ptr();
    auto root_node = allocate_leaf_node(0);
    //debug::notify_info("root node addr (node %d addr %lx)", root_node.node_id, root_node.addr);
    //printf("Global Root addr = 0x%" PRIx64 "\n", root_node.val);
    update_root_ptr(root_node, global_addr_t::null());
    leaf_num = 1;
    inner_num = 0;
    height = 1;
    // fetch_root();
    ...
    cache_num = 0; */
    //debug::notify_info("root addr (node %d addr %lx)", root.node_id, root.addr);
  ...
  BTree(int tree_id,
        uint64_t cache_mb, double sample_rate,
    ...
    if (num_partitions == 1) {
      ...
      global_addr_t newInner = allocate_node(0);
      std::cout << "--------ORG root ptr = " << newInner.node_id << " " << newInner.addr << std::endl;
      auto page_buffer = transport->get_buffer() + pageSize;
      auto new_mem = new ((void*)page_buffer) BTreeInner(1, newInner);
      new_mem->count = num_partitions - 1;
      new_mem->children[0] = allocate_leaf_node(partition_info[0], partition_info[1] - 1, 0);
      ...
      rwrite(page_buffer, newInner, pageSize);
      update_root_ptr(newInner, global_addr_t::null());
  ...
  // dummy functions
  int insert(Key k, Value v, UnstructuredBuffer& evict) { return 0; }
  int update(Key k, Value v, UnstructuredBuffer& evict) { return 0; }
  int lookup(Key k, Value& v, UnstructuredBuffer& evict) { return 0; }
  int remove(Key k, UnstructuredBuffer& evict) { return 0; } 
  int search_cache(Key k, UnstructuredBuffer& buffer) { return 0; }
  void add_to_cache(Key k, Value v) { }
  void invalidate(Key k, char* page) { }


  void rwrite(char* src, global_addr_t dest, int size){
      transport->write(src, dest, size);
  }

  void rread(char* src, global_addr_t dest, int size){
      transport->read(src, dest, size); 
  }

  bool remote_cas(global_addr_t dest, uint64_t cmp, uint64_t swap, char* cas_buffer){
      bool ret = transport->cas(reinterpret_cast<char*>(cas_buffer), dest, cmp, swap, 8);
      return ret;
  }

  void fetch_root_ptr(int node_id){
      uint32_t base_size = sizeof(message_t) - sizeof(char*);
      auto send_ptr = transport->get_buffer();
      auto send_msg = new (send_ptr) message_t(message_t::type_t::REQUEST_IDX_GET_ROOT, sizeof(tree_id_));
      UnstructuredBuffer send_buffer(send_msg->get_data_ptr());
      send_buffer.put(&tree_id_);
      transport->send(send_ptr, node_id, base_size + sizeof(tree_id_));

      auto recv_ptr = transport->get_recv_buffer(node_id);
      auto recv_msg = new (recv_ptr) message_t();
      transport->recv(recv_ptr, node_id, MAX_MESSAGE_SIZE);
      if(recv_msg->get_type() != message_t::type_t::ACK) {
	      debug::notify_error("[INDEX] fetch_root_ptr failed... return type %d", recv_msg->get_type());
        assert(false);
      }
      root_ptr_ptr_ = *reinterpret_cast<global_addr_t*>(recv_msg->get_data_ptr());
      printf("idx_root: index %u, val(%lu), addr(%lu)\n", tree_id_, root_ptr_ptr_.val, root_ptr_ptr_.addr);
      //debug::notify_info("[INDEX] fetch_root root_ptr addr (node %d addr %lx)", root_ptr_ptr_.node_id, root_ptr_ptr_.addr);
  }

  void fetch_root(){
      if(root == global_addr_t::null()){
	  auto page_buffer = transport->get_buffer();
	  rread(page_buffer, root_ptr_ptr_, sizeof(global_addr_t));
	  root = *reinterpret_cast<global_addr_t*>(page_buffer);
      }
  ...
  void update_root_ptr(global_addr_t new_root, global_addr_t old_root) {
    //  debug::notify_info("[update_root_ptr] root_ptr_ptr_ (%d %lx), new_root (%d %lx), old_root (%d %lx)", root_ptr_ptr_.node_id, root_ptr_ptr_.addr, new_root.node_id, new_root.addr, old_root.node_id, old_root.addr);
    auto cas_buffer = transport->get_buffer();
    bool res = remote_cas(root_ptr_ptr_, get_global_address(old_root).val, get_global_address(new_root).val, cas_buffer);
    if (res) {
      std::cout << "Success: tree root pointer value "
                << get_global_address(new_root).node_id << " "
		<< get_global_address(new_root).addr
		<< std::endl;
      root = new_root;
    } else {
      ...
      std::cout << "Fail: tree root pointer value from remote "
                << get_global_address(root).node_id << " "
                << get_global_address(root).addr 
		<< std::endl;
    }
  ...
  void get_newest_root() {
    assert(root_ptr_ptr_ != global_addr_t::null());
    auto page_buffer = transport->get_buffer();
    rread(page_buffer, root_ptr_ptr_, sizeof(global_addr_t));
    auto new_root = *reinterpret_cast<global_addr_t *>(page_buffer);
  ...
  uint32_t get_random_id(int id){
      ...
      static thread_local std::uniform_int_distribution<int> distribution(0, g_num_server_nodes - 1);
      auto idx = distribution(*generator);
      if(g_num_server_nodes > 1){
	      while(idx == id)
  ...
  void makeRoot(Key k, global_addr_t leftChild, global_addr_t rightChild, uint8_t level) {
    global_addr_t new_root;
    auto remote_left = get_global_address(leftChild);
    if ((level % megaLevel) == 0) {
      // new_root = allocate_node(remote_left.node_id + 1);
      new_root = allocate_node(get_random_id(remote_left.node_id));
    } else {
      new_root = allocate_node(remote_left.node_id);
    }
    // auto page_buffer = get_sibling_buffer();
    auto page_buffer = transport->get_buffer() + pageSize;
    auto inner = new ((void*)page_buffer) BTreeInner(level, new_root);
    inner->count = 1;
    inner->keys[0] = k;
  ...
  global_addr_t get_root_ptr_ptr() {
    auto root_buffer = transport->get_buffer(); 
    rread(root_buffer, root_ptr_ptr_, sizeof(global_addr_t));
    auto addr = *reinterpret_cast<global_addr_t*>(root_buffer);
    return addr;
  }

  global_addr_t get_root_lock_ptr() {
    global_addr_t addr;
    addr.node_id = 0;
    addr.addr = root_ptr_ptr_.addr + sizeof(global_addr_t);
    return addr;
  }

  // allocate distributed node using RDMA
  global_addr_t allocate_node() {
    // global_addr_t node_addr;
    ...
    // return node_addr;
    static thread_local uint32_t next_target_node = g_node_id;
    next_target_node = ++next_target_node % g_num_server_nodes;
    return GET_WORKLOAD->rpc_alloc(next_target_node, pageSize);
  }

  global_addr_t allocate_node(uint32_t node_id) {
    // global_addr_t node_addr;
    // auto ret = global_allocator::allocate();
    // if (ret == nullptr) {
    //   std::cout << "External Mem allocation failed" << std::endl;
    ...
    // node_addr.val = reinterpret_cast<uint64_t>(ret);
    // return node_addr;
    node_id %= g_num_server_nodes;
    return GET_WORKLOAD->rpc_alloc(node_id, pageSize);
  }

  // FIXME(BT): this should be removed
  global_addr_t allocate_leaf_node() {
    auto node_addr = allocate_node();
    auto page_buffer = transport->get_buffer();
    auto new_mem = new ((void*)page_buffer) BTreeLeaf<Value>(node_addr);
    new_mem->typeVersionLockObsolete = 0;
    rwrite(page_buffer, node_addr, pageSize);
    return node_addr;
  }

  global_addr_t allocate_leaf_node(uint32_t node_id) {
    auto node_addr = allocate_node(node_id);
    auto page_buffer = transport->get_buffer(); 
    auto new_mem = new ((void*)page_buffer) BTreeLeaf<Value>(node_addr);
    new_mem->typeVersionLockObsolete = 0;
    rwrite(page_buffer, node_addr, pageSize);
  ...
  global_addr_t allocate_leaf_node(Key min_limit, Key max_limit,
                                   uint32_t node_id = 0) {
    auto node_addr = allocate_node(node_id);
    auto page_buffer = transport->get_buffer();
    auto cur_node = new ((void*)page_buffer) BTreeLeaf<Value>(node_addr);
  ...
  void get_basic() {
    ...
    std::cout << "max. leaf entries = " << BTreeLeaf<Value>::maxEntries
              << std::endl;
    std::cout << "max. inner entries = " << BTreeInner::maxEntries
              << std::endl;

    std::cout << "Inner page size = " << sizeof(BTreeInner) << std::endl;
    std::cout << "Leaf page size = " << sizeof(BTreeLeaf<Value>)
              << std::endl;
  }

  void get_node_info() {
    std::cout << "inner node size (B) = " << sizeof(BTreeInner)
              << std::endl;
    std::cout << "leaf node size (B) = " << sizeof(BTreeLeaf<Value>)
              << std::endl;
  }
  ...
  void set_bound(Key left, Key right) {
    ...
    debug::notify_info("[leanstore_tree] left bound %lu, right bound %lu", left_bound_, right_bound_);
  }

  int get_node_ID(global_addr_t global_node) {
    uint64_t node = global_node.val;
    if (node & swizzle_tag) {
      auto mem_node = reinterpret_cast<NodeBase *>(node & swizzle_hide);
      return mem_node->remote_address.node_id;
    }
    return global_node.node_id;
  }

  global_addr_t get_global_address(global_addr_t global_node) {
    uint64_t node = global_node.val;
    if (node & swizzle_tag) {
      auto mem_node = reinterpret_cast<NodeBase *>(node & swizzle_hide);
  ...
  void *get_memory_address(global_addr_t global_node) {
    uint64_t node = global_node.val;
    if (node & swizzle_tag) {
  ...
  // here
  bool try_get_remote_lock(global_addr_t lock) {
    uint64_t tag = (static_cast<uint64_t>(g_node_id) << 32) | (GET_THD_ID + 1);
    auto cas_buffer = transport->get_buffer();
    return remote_cas(lock, 0, tag, cas_buffer);
  }

  bool test_remote_lock_set(global_addr_t lock) {
    auto page_buffer = transport->get_buffer();
    rread(page_buffer, lock, sizeof(uint64_t));
    auto lock_val = *reinterpret_cast<uint64_t *>(page_buffer);
    return (lock_val != 0);
  }

  void get_remote_lock(global_addr_t lock) {
    uint64_t tag = (static_cast<uint64_t>(g_node_id) << 32) | (GET_THD_ID + 1);
    auto cas_buffer = transport->get_buffer();
    while (true) {
      auto ret = remote_cas(lock, 0, tag, cas_buffer);
      if (ret)
        break;
    }
  }

  void release_remote_lock(global_addr_t lock) {
    auto cas_buffer = reinterpret_cast<uint64_t*>(transport->get_buffer());
    *cas_buffer = 0;
    rwrite(reinterpret_cast<char*>(cas_buffer), lock, sizeof(uint64_t));
  }

  bool is_local_remote_match(NodeBase *local_node, global_addr_t remote_node, bool verbose = false) {
  ...
  void bulk_load(Key *bulk_array, uint64_t bulk_load_num) {
    if (g_node_id == 0) {
      ...
      for (uint64_t i = 0; i < bulk_load_num; ++i) {
        ...
        // std::endl;
        insert_single(bulk_array[i], bulk_array[i]);
  ...
  global_addr_t recursive_movement_single(global_addr_t node,
                                          NodeBase *mem_node,
    ...
    if (cur_mega_level > 0) {
	  ...
	  /*
	  if(next_mem_node == nullptr)
	      return global_addr_t::null();
	      */
	...
	/*
	if(new_node == global_addr_t::null()){
	...
	*/
	...
	/*
      if (node.node_id == node_id) {
        std::cout << "Existing ID = " << node.node_id << std::endl;
  ...
  restart:
    ...
    while (mem_node->type == PageType::BTreeInner) {
	  ...
	  auto next_node_id = get_random_id(inner->remote_address.node_id);
	  //int next_node_id = get_random_id(inner->remote_address.node_id);
          global_addr_t newInner = allocate_node(next_node_id);
          //++next_node_id;
    auto page_buffer = transport->get_buffer() + pageSize;
          auto new_mem_node =
              new (page_buffer) BTreeInner(inner->level, newInner);
          ...
          auto ret = recursive_movement_single(new_mega_root,
                                    reinterpret_cast<NodeBase *>(mem_new_inner),
                                    megaLevel - 1, new_mega_root.node_id);
          //recursive_movement_single(new_mega_root,
          //                          reinterpret_cast<NodeBase *>(mem_new_inner),
          //                          megaLevel - 1, new_mega_root.node_id);
          mem_new_inner->writeUnlock();
	  /*
	  if(ret == global_addr_t::null()){
	      new_refresh_from_root(k);
	  }*/
        } else {
          Key sep;
          global_addr_t newInner =
              allocate_node(mem_node->remote_address.node_id);
          auto page_buffer = transport->get_buffer() + pageSize;
          auto new_mem =
              new ((void*)page_buffer) BTreeInner(inner->level, newInner);
          inner->writeLockOrRestart(needRestart);
          assert(needRestart == false);
    ...
    auto leaf = static_cast<BTreeLeaf<Value> *>(mem_node);
    ...
    if (leaf->count == leaf->maxEntries) {
      //  Split
      Key sep;
      auto newLeaf = allocate_node(mem_node->remote_address.node_id);
      auto page_buffer = transport->get_buffer() + pageSize;
      auto new_mem = new ((void*)page_buffer) BTreeLeaf<Value>(newLeaf);
      // leaf->pos_state = 4;
      ...
      new_mem->typeVersionLockObsolete = 0;
      rwrite(page_buffer, newLeaf, pageSize);
      ++leaf_num;
      if (parent != super_root_) {
  ...
  void remote_write_unswizzle(NodeBase *mem_node, bool clear_lock = false) {
    ...
    if (!clear_lock) {
      buffer_node->typeVersionLockObsolete = 0;
    }
  ...
  void check_parent_global_conflict(BTreeInner *parent,
                                    global_addr_t remote_addr,
    ...
    if (parent == super_root_) {
      ...
      auto remote_parent_node =
          reinterpret_cast<BTreeInner *>(raw_remote_read(remote_addr));
  ...
  void parent_read_unlock(BTreeInner *parent, uint64_t versionParent,
                          bool &needRestart) {
    if (parent == super_root_) {
      // Super root
      auto cur_root = get_global_address(root);
      ...
      if (shared) {
        auto remote_parent_node = reinterpret_cast<BTreeInner *>(
  ...
  // to this cur_node
  bool iterate_check_private(BTreeInner *inner) {
    int count = inner->count;
    bool needRestart = false;
    for (int i = 0; i <= count; ++i) {
      bool refresh = false;
  ...
  restart:
    ...
    BTreeInner *parent = reinterpret_cast<BTreeInner *>(cur_node);
    ...
    while (cur_node->type == PageType::BTreeInner) {
      auto inner = reinterpret_cast<BTreeInner *>(cur_node);
      ...
      if (cur_node->isShared()) {
        ...
        if (inner->isFull()) {
          ...
          Key sep;
#ifdef FINE_GRAINED
          global_addr_t newInner = allocate_node();
#else
          global_addr_t newInner;
          if (cur_node->level <= megaLevel - 1) {
            newInner = allocate_node(cur_node->remote_address.node_id);
          } else {
            // Allocate in other memory nodes
            thread_local uint32_t next_node_id = 0;
            // newInner = allocate_node(cur_node->remote_address.node_id + 1);
	  ...
	  auto page_buffer = transport->get_buffer() + pageSize;
          ...
          if (parent == super_root_) {
            ...
            global_addr_t node;
            node.val = reinterpret_cast<uint64_t>(inner) | swizzle_tag;
            remote_write_unswizzle(mem_new_inner);
  ...
  bool verify_and_acquire_lock(NodeBase *cur_node, uint64_t org_version,
    ...
    assert(reinterpret_cast<BTreeInner *>(remote_cur_node)
               ->check_consistent());

    // If this node is already full, we also require
    // the refresh from root
    if (reinterpret_cast<BTreeInner *>(remote_cur_node)->isFull()) {
      full_unlock(cur_node);
      refresh_cache = true;
      return false;
    }
  ...
  void new_swizzling(global_addr_t &global_addr, NodeBase *parent,
                     unsigned child_idx, NodeBase *child) {
    if (!check_parent_child_info(parent, child)) {
      std::cout << "Global addr = (" << global_addr.node_id << " " << global_addr.addr << ")" << std::endl;
    ...
    auto inner_parent = reinterpret_cast<BTreeInner *>(parent);
  ...
  NodeBase *new_get_mem_node(global_addr_t &global_addr, NodeBase *parent,
                             unsigned child_idx, uint64_t versionNode,
                             bool &restart, bool &refresh, bool IO_enable) {
    global_addr_t node = global_addr;
    ...
    global_addr_t snapshot = global_addr;
    ...
    // If IO is not supported, and IO flag is successfully inserted
    ...
    parent->IOUnlock();
    if(target_node != nullptr){
	if((uint64_t)target_node & swizzle_tag){
	    debug::notify_info("swizzle tag here!!");
	    while(1) ;
	}
    }
    return target_node;
  ...
  NodeBase *new_get_mem_node_without_lock(global_addr_t &global_addr,
  ...
  // #else
  //       auto newLeaf = allocate_node(cur_node->remote_address.node_id);
  // #endif
  //       auto page_buffer = (dsm_->get_rbuf(0)).get_sibling_buffer();
  ...
  void insert(Key k, Value v){
      bool ret = insert_(k, v);
      assert(ret == true);
  }

  bool insert_(Key k, Value v) {
    int restartCount = 0;
    bool verbose = false;
  ...
  restart:
    ...
    while (cur_node->type == PageType::BTreeInner) {
      auto inner = static_cast<BTreeInner *>(cur_node);
      // SMO logic with megaNode
...
// We don't do the online migration between memory nodes
      ...
      if (cur_node == nullptr) {
        ...
        if (inner->level == 1) {
          ...
          remote_flag = cache.cold_to_hot_with_admission(
              ...
              idx, refresh, k, v, insert_success, message_t::type_t::REQUEST_DEX_INSERT);
          if (remote_flag == 1) {
            inner->IOUnlock();
    ...
    // Access the leaf node
    BTreeLeaf<Value> *leaf =
        static_cast<BTreeLeaf<Value> *>(cur_node);
...
#endif
      auto page_buffer = transport->get_buffer() + pageSize;
      auto new_mem = new (page_buffer) BTreeLeaf<Value>(newLeaf);
      leaf->split(sep, new_mem, newLeaf);
      remote_write(newLeaf, new_mem, true, true);
    ...
    while (cur_node->type == PageType::BTreeInner) {
      ...
      if (cur_node == nullptr) {
        ...
        if (inner->level == 1) {
          ...
          remote_flag = cache.cold_to_hot_with_admission(
              inner->children[idx], reinterpret_cast<void **>(&cur_node), inner,
              idx, refresh, k, result, lookup_success, message_t::type_t::REQUEST_DEX_LOOKUP);
          if (remote_flag == 1) {
            inner->IOUnlock();
      ...
      if((uint64_t)cur_node & swizzle_tag){
	  debugging = 3;
      }
      //cur_node = (NodeBase*)(inner->children[idx].val & swizzle_hide);
      versionNode = cur_node->readLockOrRestart(needRestart);
      if (needRestart)
        goto restart;
    }
  ...
  int scan(Key k, int num, Value *&v_buffer) {
    int restartCount = 0;
    int cur_num = 0;
  restart:
    if (restartCount++)
      ...
      if (cur_node == nullptr) {
        ...
        if (inner->level == 1) {
          ...
          remote_flag = cache.cold_to_hot_with_admission_for_scan(
              inner->children[idx], reinterpret_cast<void **>(&cur_node), inner,
              idx, refresh, k, v_buffer, scan_num, max_key);
    ...
    auto ret_num = leaf->range_scan(k, num - cur_num, v_buffer);
    ...
    while (cur_node->type == PageType::BTreeInner) {
      ...
      if (cur_node == nullptr) {
        ...
        if (inner->level == 1) {
          ...
          remote_flag = cache.cold_to_hot_with_admission(
              inner->children[idx], reinterpret_cast<void **>(&cur_node), inner,
              idx, refresh, k, v, update_success, message_t::type_t::REQUEST_DEX_UPDATE);
          if (remote_flag == 1) {
            inner->IOUnlock();
    ...
    while (cur_node->type == PageType::BTreeInner) {
      ...
      if (cur_node == nullptr) {
        ...
        if (inner->level == 1) {
          ...
          remote_flag = cache.cold_to_hot_with_admission(
              inner->children[idx], reinterpret_cast<void **>(&cur_node), inner,
              idx, refresh, k, v, remove_success, message_t::type_t::REQUEST_DEX_DELETE);
          if (remote_flag == 1) {
            inner->IOUnlock();
  ...
  void validate() {
    ...
    bool ret = recursive_validate(get_global_address(root),
                                  global_addr_t::null(), false, node_num);
    if (ret == false) {
  ...
  bool recursive_validate(global_addr_t cur_node, global_addr_t parent,
                          bool validate_parent,
                          std::vector<uint64_t> &node_num) {
    node_num[cur_node.node_id]++;
    assert(cur_node != global_addr_t::null());
    if (validate_parent && cur_node.node_id != parent.node_id) {
      std::cout << "Fail from this one: cur node ID = " << cur_node.node_id
                << "; parent node ID = " << parent.node_id << std::endl;
      return false;
    }
    ...
    for (int i = 0; i <= num; ++i) {
      ...
      if (ret == false) {
        std::cout << "Recursive fail: cur node ID = " << cur_node.node_id
                  << "; parent node ID = " << parent.node_id
