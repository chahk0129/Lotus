...
int leanstore_lookup(global_addr_t root, Key k, Value &v_result,
           global_addr_t &g_result) {
  auto node_id = root.node_id;
  auto node = root;
  NodeBase *mem_node = reinterpret_cast<NodeBase *>(root.addr);
  while (mem_node->type == PageType::BTreeInner) {
    ...
    if (node.node_id != node_id) {
      ...
      mem_node = reinterpret_cast<NodeBase *>(node.addr);
    }
  }

  BTreeLeaf<Value> *leaf = static_cast<BTreeLeaf<Value> *>(mem_node);
...
// update nothing
template <class Value>
int leanstore_update(global_addr_t &root, Key k, Value v_result) {
  auto node_id = root.node_id;
  auto node = root;
  NodeBase *mem_node = reinterpret_cast<NodeBase *>(root.addr);
  while (mem_node->type == PageType::BTreeInner) {
...
template <class Value>
int leanstore_insert(global_addr_t &root, Key k, Value v_result) {
  auto node_id = root.node_id;
  auto node = root;
  NodeBase *mem_node = reinterpret_cast<NodeBase *>(root.addr);
  while (mem_node->type == PageType::BTreeInner) {
...
// remove nothing
template <class Value>
int leanstore_remove(global_addr_t &root, Key k) {
  auto node_id = root.node_id;
  auto node = root;
  NodeBase *mem_node = reinterpret_cast<NodeBase *>(root.addr);
  while (mem_node->type == PageType::BTreeInner) {
