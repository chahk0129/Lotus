...
public:
  ...
  page_bucket *get_bucket(global_addr_t key) {
    auto hash_val =
        hash(reinterpret_cast<void *>(&(key.val)), sizeof(global_addr_t));
    auto bucket_idx = hash_val % bucket_num_;
    return (table_ + bucket_idx);
  }

  bool insert(page_bucket *head_bucket, global_addr_t key, void *&value) {
  ...
  bool insert_io_flag(page_bucket *head_bucket, global_addr_t key) {
    void *value = reinterpret_cast<void *>(IO_FLAG);
    return insert(head_bucket, key, value);
  }

  bool insert_with_lock(global_addr_t key, void *&value) {
  ...
  bool insert_io_flag_with_lock(global_addr_t key) {
    auto head_bucket = get_bucket(key);
    head_bucket->lock_.get_lock();
  ...
  bool upsert(page_bucket *head_bucket, global_addr_t key, void *&value) {
    auto cur_bucket = head_bucket;
    // update
    while (cur_bucket) {
      auto cur_frame = cur_bucket->page_frame_;
  ...
  bool upsert_with_lock(global_addr_t key, void *&value) {
    auto head_bucket = get_bucket(key);
    head_bucket->lock_.get_lock();
  ...
  bool remove(page_bucket *head_bucket, global_addr_t key, void *value) {
    auto cur_bucket = head_bucket;
  ...
  bool remove_with_lock(global_addr_t key, void *value) {
    auto head_bucket = get_bucket(key);
    head_bucket->lock_.get_lock();
  ...
  bool remove(page_bucket *head_bucket, global_addr_t key) {
    auto cur_bucket = head_bucket;
    page_bucket *prev_bucket = nullptr;
    while (cur_bucket) {
      auto cur_frame = cur_bucket->page_frame_;
  ...
  // The opt lock
  bool check_and_remove(global_addr_t key) {
    auto head_bucket = get_bucket(key);
  ...
  void *get(page_bucket *head_bucket, global_addr_t key) {
    auto cur_bucket = head_bucket;
    while (cur_bucket) {
      auto cur_frame = cur_bucket->page_frame_;
      for (int i = 0; i < num_entry_in_page_bucket; ++i) {
  ...
  void *get_with_lock(global_addr_t key) {
    auto head_bucket = get_bucket(key);
  ...
  void *get_with_lock(page_bucket *head_bucket, global_addr_t key) {
    void *ret = nullptr;
    while (true) {
      uint32_t version = 0;
      auto flag = head_bucket->lock_.test_lock_set(version);
  ...
  bool update(page_bucket *head_bucket, global_addr_t key, void *new_value,
              void **old_value) {
    auto cur_bucket = head_bucket;
    while (cur_bucket) {
      auto cur_frame = cur_bucket->page_frame_;
  ...
  bool update_with_lock(global_addr_t key, void *new_value, void **old_value) {
    auto head_bucket = get_bucket(key);
    head_bucket->lock_.get_lock();
  ...
  bool RMW(page_bucket *head_bucket, global_addr_t key, void *new_value,
           void *old_value) {
    auto cur_bucket = head_bucket;
    while (cur_bucket) {
      auto cur_frame = cur_bucket->page_frame_;
  ...
  bool RMW_with_lock(global_addr_t key, void *new_value, void *old_value) {
    auto head_bucket = get_bucket(key);
    head_bucket->lock_.get_lock();
