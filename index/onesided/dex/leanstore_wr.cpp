...
NodeBase *opt_remote_read(global_addr_t global_node) {
  ...
  transport->read(page_buffer, global_node, 16);
  ...
  transport->read(page_buffer, global_node, pageSize);
  mem_node = reinterpret_cast<NodeBase *>(page_buffer);

  // 3. version verification
  auto new_page_buffer = transport->get_buffer();
  transport->read(new_page_buffer, global_node, 16);
...
char *raw_remote_read(global_addr_t global_node, size_t read_size) {
  auto page_buffer = transport->get_buffer(); 
  transport->read(page_buffer, global_node, read_size);
  return page_buffer;
}
...
NodeBase *raw_remote_read_by_sibling(global_addr_t global_node) {
  auto page_buffer = transport->get_buffer() + pageSize; // sibling buffer
  transport->read(page_buffer, global_node, pageSize);
  auto mem_node = reinterpret_cast<NodeBase *>(page_buffer);
  return mem_node;
}
...
void remote_write(global_addr_t global_node, NodeBase *mem_node, 
                  bool clear_lock, bool without_copy) {
  //++write_counter;
  ...
  if (without_copy) {
    ...
    page_buffer = transport->get_buffer();
  ...
  transport->write(page_buffer, global_node, pageSize);
...
NodeBase *opt_remote_read_by_sibling(global_addr_t global_node) {
  ...
  // 3. version verification
  auto new_page_buffer = transport->get_buffer() + pageSize; // sibling buffer
  transport->read(new_page_buffer, global_node, 16);
  NodeBase *new_mem_node = reinterpret_cast<NodeBase *>(new_page_buffer);
  if ((new_mem_node->typeVersionLockObsolete != 0) ||
      (new_mem_node->front_version != org_version)) {
...
void remote_write_by_sibling(global_addr_t global_node, NodeBase *mem_node, bool clear_lock) {
  //++write_counter;
  auto page_buffer = transport->get_buffer() + pageSize; // sibling buffer
  mem_node->dirty = false;
  if (page_buffer != reinterpret_cast<char*>(mem_node)) {
  ...
  if (clear_lock)
    flushed_page->typeVersionLockObsolete = 0;
  transport->write(page_buffer, global_node.addr, pageSize);
}
...
bool check_limit_match(NodeBase *parent, NodeBase *child,
                       bool using_swizzle, bool verbose) {
