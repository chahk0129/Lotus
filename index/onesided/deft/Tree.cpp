#include "index/onesided/deft/Tree.h"
#include "index/onesided/deft/IndexCache.h"
#include "index/onesided/deft/Timer.h"
...
Tree::Tree(uint32_t tree_id, uint64_t index_cache_size) 
    : tree_id(tree_id), root_ptr_ptr(global_addr_t::null()), 
      g_root_ptr(global_addr_t::null()), g_root_level(-1) {
  ...
  if (index_cache_size != 0 ) {
    uint64_t cache_size = index_cache_size * 1024 * 1024; // MB to B
    index_cache = new IndexCache(cache_size);
  } 

  root_ptr_ptr = global_addr_t::null();
  if (g_node_id == 0) // master compute server initializes root
    initializeRoot();
  while (root_ptr_ptr == global_addr_t::null()) root_ptr_ptr = get_root_ptr_ptr();
}
...
void Tree::initializeRoot() {
  ...
  global_addr_t root_ptr_addr = GET_WORKLOAD->get_root(tree_id, 0);
  assert(root_ptr_addr != global_addr_t::null());
  global_addr_t root_addr = GET_WORKLOAD->rpc_alloc(kLeafPageSize);
  [[maybe_unused]] LeafPage* root_page = new (page_buffer) LeafPage();
  remote_write(page_buffer, root_addr, kLeafPageSize);
  // transport->write(page_buffer, root_addr, kLeafPageSize);

  auto root_ptr = reinterpret_cast<global_addr_t*>(page_buffer);
  *root_ptr = root_addr;
  remote_write(page_buffer, root_ptr_addr, sizeof(global_addr_t));
  // transport->write(page_buffer, root_ptr_addr, sizeof(global_addr_t));

  // bool ret = remote_cas_mask(page_buffer, root_ptr_addr, 0, 1, ADD_S_LOCK, 3);
  // assert(ret);
  // remote_faa_bound(page_buffer, root_ptr_addr, 1, ADD_S_UNLOCK, 3);
  // global_addr_t lock_addr = get_lock_addr(root_addr);
  // uint64_t *lock_buffer = reinterpret_cast<uint64_t*>(page_buffer);
  // *lock_buffer = 0;
  // uint64_t prev_lock = *lock_buffer;
  // remote_read_dm(page_buffer, lock_addr, sizeof(uint64_t));
  // assert(prev_lock == *lock_buffer);
  // *lock_buffer = 1;
  // remote_write_dm(page_buffer, lock_addr, sizeof(uint64_t));
  // remote_read_dm(page_buffer, lock_addr, sizeof(uint64_t));
  // uint64_t cur_lock = *lock_buffer;
  // assert(cur_lock == 1);
  // *lock_buffer = 0;
  // remote_write_dm(page_buffer, lock_addr, sizeof(uint64_t));
  ...
  // *lock_buffer = 1;
  // remote_faa_dm_bound(page_buffer, lock_addr, 1, ADD_S_LOCK, sizeof(uint64_t));
  // remote_read_dm(page_buffer, lock_addr, sizeof(uint64_t));
  // cur_lock = *lock_buffer;
  // printf("lock after bound %lu\n", cur_lock);
  // *lock_buffer = 0;
  // remote_write_dm(page_buffer, lock_addr, sizeof(uint64_t));
}
...
global_addr_t Tree::get_root_ptr(CoroContext *ctx, bool force_read) {
  if (force_read || g_root_ptr == global_addr_t::null()) {
    char *page_buffer = transport->get_buffer();
    remote_read(page_buffer, root_ptr_ptr, sizeof(global_addr_t), false);
    // transport->read(page_buffer, root_ptr_ptr, sizeof(global_addr_t));
    //     (dsm_client_->get_rbuf(ctx ? ctx->coro_id : 0)).get_page_buffer();
    // dsm_client_->ReadSync(page_buffer, root_ptr_ptr, sizeof(global_addr_t),
    //                       ctx);
    global_addr_t root_ptr = *(global_addr_t *)page_buffer;
    g_root_ptr = root_ptr;
    return root_ptr;
  } else {
    return g_root_ptr;
...
void Tree::remote_read(char *read_buffer, global_addr_t addr, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->read(read_buffer, addr_, size, !async);
}

void Tree::remote_read_dm(char *read_buffer, global_addr_t addr, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->read_dm(read_buffer, addr_, size, !async);
}

void Tree::remote_write(char *write_buffer, global_addr_t addr, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->write(write_buffer, addr_, size, !async);
}

void Tree::remote_write_dm(char *write_buffer, global_addr_t addr, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->write_dm(write_buffer, addr_, size, !async);
}

bool Tree::remote_cas(char *cas_buffer, global_addr_t addr, uint64_t cmp,
                      uint64_t swap, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  return transport->cas(cas_buffer, addr_, cmp, swap, size, !async);
}

bool Tree::remote_cas_mask(char *cas_buffer, global_addr_t addr, uint64_t cmp,
                           uint64_t swap, uint64_t mask, uint32_t size,
                           bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  return transport->cas_mask(cas_buffer, addr_, cmp, swap, mask, size, !async);
}

bool Tree::remote_cas_dm(char *cas_buffer, global_addr_t addr, uint64_t cmp,
                         uint64_t swap, uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  return transport->cas_dm(cas_buffer, addr_, cmp, swap, size, !async);
}

bool Tree::remote_cas_dm_mask(char *cas_buffer, global_addr_t addr,
                              uint64_t cmp, uint64_t swap, uint64_t mask,
                              uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  return transport->cas_dm_mask(cas_buffer, addr_, cmp, swap, mask, size, !async);
}

void Tree::remote_faa(char *faa_buffer, global_addr_t addr, uint64_t add,
                      uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->faa(faa_buffer, addr_, add, size, !async);
}

void Tree::remote_faa_dm(char *faa_buffer, global_addr_t addr, uint64_t add,
                         uint32_t size, bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->faa_dm(faa_buffer, addr_, add, size, !async);
}

void Tree::remote_faa_bound(char *faa_buffer, global_addr_t addr,
                            uint64_t add, uint64_t boundary, uint32_t size,
                            bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->faa_bound(faa_buffer, addr_, add, boundary, size, !async);
}

void Tree::remote_faa_dm_bound(char *faa_buffer, global_addr_t addr,
                               uint64_t add, uint64_t boundary, uint32_t size,
                               bool async) {
  global_addr_t addr_(0);
  addr_.node_id = addr.nodeID;
  addr_.addr = addr.offset;
  transport->faa_dm_bound(faa_buffer, addr_, add, boundary, size, !async);
}
...
bool Tree::update_new_root(global_addr_t left, const Key &k,
                           ...
                           global_addr_t old_root, CoroContext *ctx) {
  char* page_buffer = transport->get_buffer();
  ...
  global_addr_t new_root_addr = GET_WORKLOAD->rpc_alloc(kInternalPageSize);
  // global_addr_t new_root_addr = dsm_client_->Alloc(kInternalPageSize);

  new_root->hdr.my_addr = new_root_addr;
  remote_write(page_buffer, new_root_addr, kInternalPageSize);
  ...
  if (remote_cas((char*)cas_buffer, root_ptr_ptr, old_root, new_root_addr, sizeof(global_addr_t))) {
  // if (transport->cas((char*)cas_buffer, root_ptr_ptr, old_root, new_root_addr, sizeof(global_addr_t))) {
...
inline bool Tree::try_lock_addr(global_addr_t lock_addr, uint64_t *buf,
                                CoroContext *ctx) {
#ifdef USE_LOCAL_LOCK
  ...
  retry:
    ...
    bool res = remote_cas_dm((char*)buf, lock_addr, 0, tag, 8);
...
inline void Tree::unlock_addr(global_addr_t lock_addr, uint64_t *buf,
  ...
  *buf = 0;
  if (async) {
    remote_write_dm((char *)buf, lock_addr, sizeof(uint64_t), true);
    // transport->write_dm((char *)buf, lock_addr, sizeof(uint64_t), false);
    // dsm_client_->WriteDm((char *)buf, lock_addr, sizeof(uint64_t), false);
  } else {
    remote_write_dm((char *)buf, lock_addr, sizeof(uint64_t));
...
inline void Tree::acquire_sx_lock(global_addr_t lock_addr,
  ...
  remote_faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3);
  // transport->faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3);  
...
retry:
  if (share_lock && x_cnt == x_tic) {
    ...
    if (retry_cnt > 5000000) {
      printf(
          "Deadlock [%lu, %lu] my thread %lu coro_id %d try %d lock upgrade "
          "%d\n",
          lock_addr.nodeID, lock_addr.offset, GET_THD_ID,
    ...
    remote_read_dm((char *)lock_buffer, lock_addr, 8);
    // transport->read_dm((char *)lock_buffer, lock_addr, 8);
...
inline void Tree::release_sx_lock(global_addr_t lock_addr,
                                  uint64_t *lock_buffer, CoroContext *ctx,
                                  bool async, bool share_lock) {
  uint64_t add_val = share_lock ? ADD_S_UNLOCK : ADD_X_UNLOCK;
  if (async) {
    remote_faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3, true);
    // transport->faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3, false);
...
inline void Tree::acquire_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
...
inline void Tree::release_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
  ...
  *lock_buffer = 0;
  if (async) {
    remote_write_dm((char *)lock_buffer, lock_addr, sizeof(uint64_t), true);
    // transport->write_dm((char *)lock_buffer, lock_addr, sizeof(uint64_t), false);
    // dsm_client_->WriteDm((char *)lock_buffer, lock_addr, sizeof(uint64_t),
    //                       false);
  } else {
    remote_write_dm((char *)lock_buffer, lock_addr, sizeof(uint64_t));
    // transport->write_dm((char *)lock_buffer, lock_addr, sizeof(uint64_t));
...
void Tree::write_and_unlock(char *write_buffer, global_addr_t write_addr,
                            int write_size, uint64_t *cas_buffer,
                            global_addr_t lock_addr, CoroContext *ctx,
...
#ifdef USE_SX_LOCK
  remote_write((char*)write_buffer, write_addr, write_size, true);
  // transport->write(write_buffer, write_addr, write_size, false);
  ...
  bool hand_over_other = can_hand_over(lock_addr);
  if (hand_over_other) {
    remote_write((char*)write_buffer, write_addr, write_size);
    // transport->write(write_buffer, write_addr, write_size);
  ...
  uint64_t* cas_buf = (uint64_t*)cas_buffer;
  *cas_buf = 0;
  if (async) {
    remote_write((char*)write_buffer, write_addr, write_size, true);
    remote_write_dm((char*)cas_buffer, lock_addr, sizeof(uint64_t), true);
    // transport->write(write_buffer, write_addr, write_size, false);
    // transport->write_dm(cas_buffer, lock_addr, sizeof(uint64_t), false);
    // dsm_client_->WriteBatch(rs, 2, false);
  } else {
    remote_write((char*)write_buffer, write_addr, write_size, true);
    remote_write_dm((char*)cas_buffer, lock_addr, sizeof(uint64_t));
    // transport->write(write_buffer, write_addr, write_size, false);
    // transport->write_dm(cas_buffer, lock_addr, sizeof(uint64_t));
...
#endif
  auto t = timer.end();
  stat_helper.add(GET_THD_ID, lat_write_page, t);
}

void Tree::cas_and_unlock(global_addr_t cas_addr, int log_cas_size,
                          uint64_t *cas_buffer, uint64_t equal, uint64_t swap,
                          uint64_t mask, global_addr_t lock_addr,
...
#ifdef USE_SX_LOCK
  remote_cas_mask((char*)cas_buffer, cas_addr, equal, swap, mask, log_cas_size, true);
  // transport->cas_mask((char*)cas_buffer, cas_addr, equal, swap, mask, log_cas_size, false);
  ...
  bool hand_over_other = can_hand_over(lock_addr);
  if (hand_over_other) {
    remote_cas_mask((char*)cas_buffer, cas_addr, equal, swap, mask, log_cas_size);
    // transport->cas_mask((char*)cas_buffer, cas_addr, equal, swap, mask, log_cas_size);
  ...
  // rs[1].is_on_chip = true;
  // *(uint64_t *)rs[1].source = 0;
  uint64_t* lock_buf = (uint64_t*)lock_buffer;
  *lock_buf = 0;
  if (async) {
    remote_cas_mask(cas_buffer, cas_addr, equal, swap, mask, log_cas_size, true);
    remote_write_dm((char*)lock_buffer, lock_addr, sizeof(uint64_t), true);
    // transport->cas_mask(cas_buffer, cas_addr, equal, swap, mask, log_cas_size, false);
    // transport->write_dm(lock_buffer, lock_addr, sizeof(uint64_t), false);
    // dsm_client_->CasMaskWrite(rs[0], equal, swap, mask, rs[1], false);
  } else {
    remote_cas_mask(cas_buffer, cas_addr, equal, swap, mask, log_cas_size, true);
    remote_write_dm((char*)lock_buffer, lock_addr, sizeof(uint64_t));
    // transport->cas_mask(cas_buffer, cas_addr, equal, swap, mask, log_cas_size, false);
    // transport->write_dm(lock_buffer, lock_addr, sizeof(uint64_t), true);
...
void Tree::lock_and_read(global_addr_t lock_addr, bool share_lock,
                         bool upgrade_from_s, uint64_t *lock_buffer,
                         global_addr_t read_addr, int read_size,
  ...
  remote_faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3, true);
  remote_read(read_buffer, read_addr, read_size);
  // transport->faa_dm_bound((char*)lock_buffer, lock_addr, add_val, XS_LOCK_FAA_MASK, 3, false);
  // transport->read(read_buffer, read_addr, read_size);
...
retry:
  if (share_lock && x_cnt == x_tic) {
    ...
    remote_read_dm((char *)lock_buffer, lock_addr, 8, true);
    remote_read(read_buffer, read_addr, read_size);
    // transport->read_dm((char *)lock_buffer, lock_addr, 8, false);
    // transport->read(read_buffer, read_addr, read_size);
  ...
  retry:
    ...
    if (retry_cnt > 5000000) {
      printf("Deadlock [%lu, %lu] my thread %lu coro_id %d\n", lock_addr.nodeID,
             lock_addr.offset, GET_THD_ID,
             ctx ? ctx->coro_id : 0);
      fflush(stdout);
      assert(false);
      exit(-1);
    ...
    Timer timer;
    timer.begin();
    bool res = remote_cas_dm((char*)lock_buffer, lock_addr, 0, tag, 8);
    remote_read(read_buffer, read_addr, read_size);
    // bool res = transport->cas_dm((char*)lock_buffer, lock_addr, 0, tag, 8, false);
    // transport->read(read_buffer, read_addr, read_size);
    // bool res = dsm_client_->CasReadSync(rs[0], rs[1], 0, tag, ctx);
    uint64_t t = timer.end();
    stat_helper.add(GET_THD_ID, lat_read_page, t);
    if (!res) {
      // conflict_tag = *buf - 1;
  ...
  remote_read((char*)read_buffer, read_addr, read_size);
  // transport->read(read_buffer, read_addr, read_size);
  // dsm_client_->ReadSync(read_buffer, read_addr, read_size, ctx);
  uint64_t t = timer.end();
  stat_helper.add(GET_THD_ID, lat_read_page, t);
...
void Tree::insert_internal_update_left_child(const Key &k, global_addr_t v,
                                             const Key &left_child,
                                             global_addr_t left_child_val,
                                             CoroContext *ctx, int level) {
  assert(left_child_val != global_addr_t::null());
...
next:
  ...
  if (result.sibling != global_addr_t::null()) {
    p = result.sibling;
...
void Tree::insert(const Key &k, const Value &v, CoroContext *ctx) {
  ...
  if (index_cache) {
    global_addr_t cache_addr, parent_addr;
    ...
    if (entry) {  // cache hit
      ...
      index_cache->invalidate(entry, GET_THD_ID);
    }
...
next:
  ...
  if (result.is_leaf) {
    ...
    if (result.sibling != global_addr_t::null()) {  // turn right
      p = result.sibling;
      min = result.next_min; // remain the max
      level_hint = 0;
      goto next;
...
int Tree::range_query(const Key &from, const Key &to, Value *value_buffer,
  ...
  for (auto page : result) {
    ...
    for (int i = 0; i < kInternalCardinality; ++i) {
      if (page->records[i].ptr != global_addr_t::null()) {
    ...
    if (cnt > 0) {
      if (page->hdr.leftmost_ptr != global_addr_t::null()) {
  ...
  for (size_t i = 0; i < leaves.size(); ++i) {
    ...
    remote_read(range_buffer + kLeafPageSize * (i % kParaFetch),
                     leaves[i], kLeafPageSize);
    // transport->read(range_buffer + kLeafPageSize * (i % kParaFetch),
    //                  leaves[i], kLeafPageSize);
...
bool Tree::leaf_page_group_search(global_addr_t page_addr, const Key &k,
                                  SearchResult &result, CoroContext *ctx,
                                  bool from_cache) {
  char *page_buffer =
      transport->get_buffer();
      // (dsm_client_->get_rbuf(ctx ? ctx->coro_id : 0)).get_page_buffer();
...
re_read:
  ...
  if (has_header) {
    remote_read(page_buffer + bucket_offset,
                     GADD(page_addr, bucket_offset), kReadBucketSize, true);
    // transport->read(page_buffer + bucket_offset,
    //                  GADD(page_addr, bucket_offset), kReadBucketSize), false;
    // dsm_client_->Read(page_buffer + bucket_offset,
    //                   GADD(page_addr, bucket_offset), kReadBucketSize, false);
    // read header
    remote_read(page_buffer + header_offset,
                     GADD(page_addr, header_offset), sizeof(Header));
    // transport->read(page_buffer + header_offset,
    //                  GADD(page_addr, header_offset), sizeof(Header));
...
bool Tree::page_search(global_addr_t page_addr, int level_hint, int read_gran,
...
re_read:
  ...
  if (read_gran == gran_full) {
    has_header = true;
    remote_read(page_buffer, page_addr,
                     std::max(kInternalPageSize, kLeafPageSize));
    // transport->read(page_buffer, page_addr,
    //                  std::max(kInternalPageSize, kLeafPageSize));
  ...
  stat_helper.add(GET_THD_ID, lat_crc, t);
  stat_helper.add(GET_THD_ID, lat_cache_search, c);
#endif
  } else {
    ...
    // dsm_client_->ReadSync(page_buffer, page_addr,
    //                       std::max(kInternalPageSize, kLeafPageSize), ctx);
    remote_read(page_buffer + start_offset, 
                     GADD(page_addr, start_offset),
                     internal_read_cnt * sizeof(InternalEntry));
    // transport->read(page_buffer + start_offset,
    //                  GADD(page_addr, start_offset),
    //                  internal_read_cnt * sizeof(InternalEntry));
  ...
  if (result.is_leaf) {
    ...
    assert(result.sibling != global_addr_t::null() ||
           result.next_level != global_addr_t::null());
    if (result.level == 1 && index_cache) {
      if (read_gran == gran_full) {
        index_cache->add_to_cache(page, GET_THD_ID);
      } else {
    ...
    // auto t = timer.end();
    // stat_helper.add(GET_THD_ID, lat_internal_search, t);
  }

  return true;
}

void Tree::update_ptr_internal(const Key &k, global_addr_t v, CoroContext *ctx,
                               int level) {
  global_addr_t root = get_root_ptr(ctx);
  SearchResult result;
...
inline void Tree::internal_page_slice_search(InternalEntry *entries, int cnt,
  ...
  for (int i = 0; i < cnt; ++i) {
    if (entries[i].ptr != global_addr_t::null()) {
      if (entries[i].key <= k) {
...
void Tree::internal_page_update(global_addr_t page_addr, const Key &k,
                                global_addr_t value, int level,
                                CoroContext *ctx, bool sx_lock) {
  global_addr_t lock_addr = get_lock_addr(page_addr);

  char* page_buffer = transport->get_buffer();
  uint64_t* lock_buffer = reinterpret_cast<uint64_t*>(page_buffer + kInternalPageSize);
  // auto &rbuf = dsm_client_->get_rbuf(ctx ? ctx->coro_id : 0);
  ...
  if (k == page->hdr.lowest) {
    ...
    write_and_unlock(modfiy, GADD(page_addr, (modfiy - page_buffer)),
                     sizeof(global_addr_t), lock_buffer, lock_addr, ctx, true,
                     sx_lock);
    return;
  } else {
    ...
    for (int i = begin_idx; i < end_idx; ++i) {
      if (page->records[i].ptr == value && page->records[i].key == k) {
        ...
        write_and_unlock(modify, GADD(page_addr, (modify - page_buffer)),
                         sizeof(global_addr_t), lock_buffer, lock_addr, ctx,
                         true, sx_lock);
        return;
      }
    }
...
void Tree::internal_page_store_update_left_child(global_addr_t page_addr,
                                                 const Key &k, global_addr_t v,
                                                 const Key &left_child,
                                                 global_addr_t left_child_val,
                                                 global_addr_t root, int level,
                                                 CoroContext *ctx) {
  global_addr_t lock_addr = get_lock_addr(page_addr);
  ...
  if (left_child_val != global_addr_t::null() &&
  ...
  if (k >= page->hdr.highest) {
    ...
    if (left_child_val != global_addr_t::null()) {
      ...
      if (modify) {
        remote_write((char *)modify,
                        GADD(page_addr, ((char *)modify - page_buffer)),
                        sizeof(global_addr_t), true);
        // transport->write((char *)modify,
        //                 GADD(page_addr, ((char *)modify - page_buffer)),
        //                 sizeof(global_addr_t), false);
  ...
  if (insert_addr) {  // has empty slot
    uint64_t *mask_buffer = lock_buffer + 1;
    ...
    if (k > page->records[last_idx].key) {
      ...
      uint64_t *old_buffer = mask_buffer + 2;
      ...
      uint64_t *cas_ret_buffer = old_buffer + 1;
      // uint64_t *cas_ret_buffer = rbuf.get_cas_buffer();
      // lock-based, must cas succeed
#if KEY_SIZE == 8
      remote_cas_mask((char *)cas_ret_buffer,
                           GADD(page_addr, ((char *)insert_addr - page_buffer)),
                           (uint64_t)old_buffer, (uint64_t)insert_addr,
                           (uint64_t)mask_buffer, 4, true);
      // transport->cas_mask((char *)cas_ret_buffer,
      //                      GADD(page_addr, ((char *)insert_addr - page_buffer)),
      //                      (uint64_t)old_buffer, (uint64_t)insert_addr,
      //                      (uint64_t)mask_buffer, 4, false);
...
#else
      remote_write((char *)&insert_addr->key,
                       GADD(page_addr, (char *)&insert_addr->key - page_buffer),
                       sizeof(InternalKey), true);
      // transport->write((char *)&insert_addr->key,
      //                 GADD(page_addr, (char *)&insert_addr->key - page_buffer),
      ...
      remote_cas((char *)cas_ret_buffer,
                       GADD(page_addr, (char *)&insert_addr->ptr - page_buffer),
                       old_entry->ptr.val, insert_addr->ptr.val,
                       sizeof(global_addr_t), true);
      // transport->cas((char *)cas_ret_buffer,
      //                  GADD(page_addr, (char *)&insert_addr->ptr - page_buffer),
      //                  old_entry->ptr.val, insert_addr->ptr.val,
      //                  sizeof(global_addr_t), false);
    ...
    // uint64_t *cas_ret_buffer = rbuf.get_cas_buffer();
    uint64_t *cas_ret_buffer = lock_buffer + 5;
#if KEY_SIZE == 8
    uint64_t *old_buffer = cas_ret_buffer + 1;
    // uint64_t *old_buffer = rbuf.get_cas_buffer();
    memcpy(old_buffer, insert_addr, sizeof(InternalEntry));
    insert_addr->key = k;
    insert_addr->ptr = v;
...
#else
    insert_addr->key = k;
    insert_addr->ptr = global_addr_t::null();
    remote_write((char *)insert_addr,
                     GADD(page_addr, (char *)insert_addr - page_buffer),
                     sizeof(InternalEntry), true);
    // transport->write((char *)insert_addr,
    //                  GADD(page_addr, (char *)insert_addr - page_buffer),
    //                  sizeof(InternalEntry), false);
    // dsm_client_->Write((char *)insert_addr,
    //                    GADD(page_addr, ((char *)insert_addr - page_buffer)),
    //                    sizeof(InternalEntry), false);
    global_addr_t new_ptr;
    new_ptr.val = insert_addr->ptr.val;
    ...
    remote_cas((char *)cas_ret_buffer,
                     GADD(page_addr, (char *)&insert_addr->ptr - page_buffer),
                     insert_addr->ptr.val, new_ptr.val, sizeof(global_addr_t),
                     true);
    // transport->cas((char *)cas_ret_buffer,
    //                  GADD(page_addr, (char *)&insert_addr->ptr - page_buffer),
    //                  insert_addr->ptr.val, new_ptr.val, sizeof(global_addr_t),
    ...
    if (left_update_addr) {
      uint64_t *cas_ret_buffer2 = cas_ret_buffer + 3;
      // uint64_t *cas_ret_buffer = rbuf.get_cas_buffer();
      uint64_t old = left_update_addr->ptr.val;
      //uint64_t old = left_update_addr->ptr.raw;
      left_update_addr->ptr = left_child_val;
      remote_cas((char *)cas_ret_buffer2,
                       GADD(page_addr, ((char *)&(left_update_addr->ptr) - page_buffer)),
                       old, left_update_addr->ptr.val, sizeof(global_addr_t),
                       true);
      // transport->cas((char *)cas_ret_buffer2,
      //                  GADD(page_addr, ((char *)&(left_update_addr->ptr) - page_buffer)),
      //                  old, left_update_addr->ptr.val, sizeof(global_addr_t),
      //                  false);
      // dsm_client_->Cas(
    ...
    if (new_gran != cur_gran) {
      ...
      global_addr_t up_level = path_stack[ctx ? ctx->coro_id : 0][level + 1];
    ...
    if (level == 1 && index_cache) {
      index_cache->add_to_cache(page, GET_THD_ID);
    }
  } else {
    // need split and insert
    global_addr_t sibling_addr = GET_WORKLOAD->rpc_alloc(kInternalPageSize);
    // global_addr_t sibling_addr = dsm_client_->Alloc(kInternalPageSize);
    char *sibling_buf = reinterpret_cast<char*>(lock_buffer + 2);
    ...
    global_addr_t split_val = tmp_records[m].ptr;
    ...
    remote_write(sibling_buf, sibling_addr, kInternalPageSize);
...
bool Tree::leaf_page_store(global_addr_t page_addr, const Key &k,
  ...
  char* page_buffer = transport->get_buffer();
  uint64_t* lock_buffer = reinterpret_cast<uint64_t*>(page_buffer + kLeafPageSize);
  uint64_t* cas_ret_buffer = lock_buffer + 1;
  // auto &rbuf = dsm_client_->get_rbuf(ctx ? ctx->coro_id : 0);
  // uint64_t *lock_buffer = rbuf.get_cas_buffer();
  // uint64_t *cas_ret_buffer = rbuf.get_cas_buffer();
  // char *page_buffer = rbuf.get_page_buffer();
  ...
  if (!group->check_consistency(!(bucket_id % 2), page_addr.node_version,
    ...
    if (from_cache) {
      ...
      remote_read(page_buffer + offsetof(LeafPage, hdr),
                       GADD(page_addr, offsetof(LeafPage, hdr)), sizeof(Header));
      // transport->read(page_buffer + offsetof(LeafPage, hdr),
      //                  GADD(page_addr, offsetof(LeafPage, hdr)), sizeof(Header));
...
retry_insert:
  ...
  if (update_addr) {
    ...
    cas_and_unlock(GADD(page_addr, ((char *)&update_addr->lv - page_buffer)),
                   3, cas_ret_buffer, update_addr->lv.val, cas_val.val, ~0ull,
...
#if KEY_SIZE == 8
    uint64_t *swap_buffer = lock_buffer + 2;
    ...
    swap_entry->lv.val = v;
    uint64_t *mask_buffer = swap_buffer + 2;
    // uint64_t *mask_buffer = rbuf.get_cas_buffer();
    mask_buffer[0] = mask_buffer[1] = ~0ull;
    bool cas_ok = remote_cas_mask((char *)cas_ret_buffer,
        GADD(page_addr, ((char *)insert_addr - page_buffer)), (uint64_t)insert_addr,
        (uint64_t)swap_buffer, (uint64_t)mask_buffer, 4, false);
    // bool cas_ok = transport->cas_mask((char *)cas_ret_buffer,
    //     GADD(page_addr, ((char *)insert_addr - page_buffer)), (uint64_t)insert_addr,
    //     (uint64_t)swap_buffer, (uint64_t)mask_buffer, 4);
    ...
    insert_addr->lv.val = __bswap_64(cas_ret_buffer[1]);
    // insert_addr->lv.raw = __bswap_64(cas_ret_buffer[1]);
#else
    LeafValue cas_val(insert_addr->lv.cl_ver, -1);
    bool cas_ok = remote_cas((char *)cas_ret_buffer,
                     GADD(page_addr, ((char *)&insert_addr->lv - page_buffer)),
                     insert_addr->lv.val, cas_val.val, sizeof(uint64_t));
    // bool cas_ok = transport->cas((char *)cas_ret_buffer,
    //                  GADD(page_addr, ((char *)&insert_addr->lv - page_buffer)),
    //                  insert_addr->lv.val, cas_val.val, sizeof(uint64_t), false);
    // bool cas_ok = dsm_client_->CasSync(
    //     GADD(page_addr, ((char *)&insert_addr->lv - page_buffer)),
    ...
    if (cas_ok) {
      ...
      cas_and_unlock(GADD(page_addr, ((char *)&insert_addr->lv - page_buffer)),
                     3, cas_ret_buffer, insert_addr->lv.val, cas_val.val, ~0ull,
                    //  3, cas_ret_buffer, insert_addr->lv.raw, cas_val.raw, ~0ull,
                     lock_addr, lock_buffer, share_lock, ctx, false);
      return true;
    }
    insert_addr->lv.val = cas_ret_buffer[0];
...
retry_with_xlock:
#endif
  if (hold_x_lock) {
    remote_read(page_buffer, page_addr, kLeafPageSize);
  ...
  //   assert(header->sibling_ptr != global_addr_t::null());
  //   release_lock(lock_addr, lock_buffer, ctx, true, share_lock);
...
retry_insert_2:
  ...
  if (sibling_addr.nodeID == page_addr.nodeID) {
    remote_write(sibling_buf, sibling_addr, kLeafPageSize, true);
  }
  else {
    remote_write(sibling_buf, sibling_addr, kLeafPageSize);
  }
  // remote_write(sibling_buf, sibling_addr, kLeafPageSize);
...
#ifdef USE_LOCAL_LOCK
// Local Locks
inline bool Tree::acquire_local_lock(global_addr_t lock_addr, CoroContext *ctx,
                                     int coro_id) {
  auto &node =
      local_locks[lock_addr.nodeID][lock_addr.addr / define::kLockSize];
