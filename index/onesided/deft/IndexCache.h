...
#include "index/onesided/deft/CacheEntry.h"
#include "index/onesided/deft/Timer.h"
#include "index/onesided/deft/thread_epoch.h"
...
class IndexCache {
 public:
  IndexCache(uint64_t cache_size, uint32_t index_id=0);
  ~IndexCache();

  bool add_to_cache(InternalPage *page, int thread_id);
  bool add_sub_node(global_addr_t addr, int group_id, int granularity,
                    int guard_offset, InternalEntry *guard, int size, Key min,
                    Key max, int thread_id);
  const CacheEntry *search_from_cache(const Key &k, global_addr_t *addr,
                                      global_addr_t *parent_addr);
 ...
 private:
  ...
  DelayFreeList delay_free_lists[MAX_NUM_THREADS];
...
inline IndexCache::IndexCache(uint64_t cache_size, uint32_t index_id) : cache_size(cache_size), index_id(index_id) {
  skiplist = new CacheSkipList(cmp, &alloc, 21);
  uint64_t memory_size = define::MB * cache_size;

  thread_status = new ThreadStatus(MAX_NUM_THREADS);
...
inline bool IndexCache::add_to_cache(InternalPage *page, int thread_id) {
  ...
  assert(new_page->hdr.my_addr != global_addr_t::null());
...
inline bool IndexCache::add_sub_node(global_addr_t addr, int group_id,
  ...
  if (e && e->from == min && e->to == max - 1) {  // update sub-node
    ...
    if (ptr) {
      ...
      new_page->hdr.sibling_ptr = global_addr_t::null();
...
inline const CacheEntry *IndexCache::search_from_cache(
    const Key &k, global_addr_t *addr, global_addr_t *parent_addr) {
  ...
  if (page && k >= page->hdr.lowest && k < page->hdr.highest) {
    ...
    if (idx != -1) {
      ...
      *addr = global_addr_t::null();
    }
    if (entry->ptr && *addr != global_addr_t::null()) {
