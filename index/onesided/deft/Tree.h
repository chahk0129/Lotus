...
#include "index/onesided/deft/Common.h"
...
class IndexCache;
extern uint64_t cache_miss[MAX_NUM_THREADS][8];
extern uint64_t cache_hit[MAX_NUM_THREADS][8];
extern uint64_t latency[MAX_NUM_THREADS][LATENCY_WINDOWS];
...
struct alignas(64) StatHelper {
  uint64_t latency_[MAX_NUM_THREADS][8];
  uint64_t counter_[MAX_NUM_THREADS][8];
...
struct alignas(64) Header {
  ...
  global_addr_t my_addr = global_addr_t::null();

  global_addr_t sibling_ptr = global_addr_t::null();
  InternalKey highest = kKeyMax;

  // align as a value-key pair
  global_addr_t leftmost_ptr = global_addr_t::null();
...
// static inline uint64_t get_lock_index(global_addr_t addr) {
//   global_addr_t a;
...
static inline global_addr_t get_lock_addr(global_addr_t addr) {
  global_addr_t a;
  a.offset = addr.offset;
  a.nodeID = addr.nodeID;
 ...
 public:
  ...
  int find_empty(int begin, int cnt) {
    for (int i = begin; i < begin + cnt; ++i) {
      if (records[i].ptr == global_addr_t::null()) {
        return i;
      }
    }
    return -1;
  ...
  int find_records_not_null(const Key &k) {
    ...
    for (int i = start_idx; i < end_idx; ++i) {
      if (records[i].key == k && records[i].ptr != global_addr_t::null()) {
        return i;
      }
    }
    return -1;
  ...
  void verbose_debug() const {
    ...
    for (int i = 0; i < kInternalCardinality; ++i) {
      printf("[%lu %lu] ", (uint64_t)records[i].key, records[i].ptr.val);
...
//   void lock_bench(const Key &k, CoroContext *ctx = nullptr, int coro_id = 0);
  ...
  void bulk_load(std::vector<std::pair<Key, value_t>>& v, uint64_t num, bool sorted=true) { assert(false); }
  void bulk_load(Key* key, uint64_t num) { assert(false); }

 private:
  uint32_t tree_id;
  global_addr_t root_ptr_ptr; // the address which stores root pointer;
  global_addr_t g_root_ptr;
  int g_root_level;
  ...
  global_addr_t get_root_ptr(CoroContext *ctx, bool force_read = false);
...
//   void coro_master(CoroYield &yield, int coro_cnt);
  ...
  bool update_new_root(global_addr_t left, const Key &k, global_addr_t right,
                       int level, global_addr_t old_root, CoroContext *ctx);

  void insert_internal_update_left_child(const Key &k, global_addr_t v,
                                         const Key &left_child,
                                         global_addr_t left_child_val,
                                         CoroContext *ctx, int level);

  bool try_lock_addr(global_addr_t lock_addr, uint64_t *buf, CoroContext *ctx);
  void unlock_addr(global_addr_t lock_addr, uint64_t *buf, CoroContext *ctx,
                   bool async);
  void acquire_sx_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
                       CoroContext *ctx, bool share_lock,
                       bool upgrade_from_s = false);
  void release_sx_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
                       CoroContext *ctx, bool async, bool share_lock);
  void acquire_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
                    CoroContext *ctx, bool share_lock,
                    bool upgrade_from_s = false);
  void release_lock(global_addr_t lock_addr, uint64_t *lock_buffer,
                    CoroContext *ctx, bool async, bool share_lock);
  void write_and_unlock(char *write_buffer, global_addr_t write_addr,
                        int write_size, uint64_t *cas_buffer,
                        global_addr_t lock_addr, CoroContext *ctx, bool async,
                        bool sx_lock);
  void cas_and_unlock(global_addr_t cas_addr, int log_cas_size,
                      ...
                      bool async);
  void lock_and_read(global_addr_t lock_addr, bool share_lock,
                     bool upgrade_from_s, uint64_t *lock_buffer,
                     global_addr_t read_addr, int read_size, char *read_buffer,
                     CoroContext *ctx);

  bool leaf_page_group_search(global_addr_t page_addr, const Key &k,
                              SearchResult &result, CoroContext *ctx,
                              bool from_cache);
  bool page_search(global_addr_t page_addr, int level_hint, int read_gran,
  ...
  void update_ptr_internal(const Key &k, global_addr_t v, CoroContext *ctx,
                           int level);
  // void internal_page_store(global_addr_t page_addr, const Key &k,
  //                          global_addr_t value, global_addr_t root, int level,
  //                          CoroContext *ctx);
  void internal_page_update(global_addr_t page_addr, const Key &k,
                            global_addr_t value, int level, CoroContext *ctx,
                            bool sx_lock);
  void internal_page_store_update_left_child(global_addr_t page_addr,
                                             const Key &k, global_addr_t value,
                                             const Key &left_child,
                                             global_addr_t left_child_val,
                                             global_addr_t root, int level,
                                             CoroContext *ctx);
  // bool try_leaf_page_upsert(global_addr_t page_addr, char *page_buffer,
  //                           global_addr_t lock_addr, uint64_t *lock_buffer,
  ...
  bool leaf_page_store(global_addr_t page_addr, const Key &k, const Value &v,
                       global_addr_t root, int level, CoroContext *ctx,
  ...
  bool acquire_local_lock(global_addr_t lock_addr, CoroContext *ctx,
                          int coro_id);
  bool can_hand_over(global_addr_t lock_addr);
  void releases_local_lock(global_addr_t lock_addr);

  void remote_read(char* buffer, global_addr_t addr, uint32_t size, bool async=false);
  void remote_read_dm(char* buffer, global_addr_t addr, uint32_t size, bool async=false);
  void remote_write(char* buffer, global_addr_t addr, uint32_t size, bool async=false);
  void remote_write_dm(char* buffer, global_addr_t addr, uint32_t size, bool async=false);
  bool remote_cas(char* buffer, global_addr_t addr, uint64_t expected, uint64_t desired,
                  uint32_t size, bool async=false);
  bool remote_cas_mask(char* buffer, global_addr_t addr, uint64_t expected, uint64_t desired,
                  uint64_t mask, uint32_t size, bool async=false);
  bool remote_cas_dm(char* buffer, global_addr_t addr, uint64_t expected, uint64_t desired,
                     uint32_t size, bool async=false);
  bool remote_cas_dm_mask(char* buffer, global_addr_t addr, uint64_t expected, uint64_t desired,
                     uint64_t mask, uint32_t size, bool async=false);
  void remote_faa(char* buffer, global_addr_t addr, uint64_t val, uint32_t size, bool async=false);
  void remote_faa_dm(char* buffer, global_addr_t addr, uint64_t val, uint32_t size, bool async=false);
  void remote_faa_bound(char* buffer, global_addr_t addr, uint64_t val, uint64_t bound,
                        uint32_t size, bool async=false);
  void remote_faa_dm_bound(char* buffer, global_addr_t addr, uint64_t val, uint64_t bound,
                           uint32_t size, bool async=false);
};
}
#endif