...
output_dir_tput="$output_dir/traffic"
mkdir -p $output_dir
...
for wk_name in "${!workloads[@]}"; do
    ...
    for cc in $cc_one; do
        param="$onesided_config $wk NUM_CLIENT_THREADS=$thd CC_ALG=$cc"
        run_experiment "$param"
    done

    for cc in $cc_two; do
        param="$twosided_config $wk NUM_CLIENT_THREADS=$thd CC_ALG=$cc BATCH=1"
        run_experiment "$param"
    done
