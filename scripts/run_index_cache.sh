...
output_dir="outputs/index_cache"
...
cache_size="32 64 128 256 512 1024 2048"
...
for wk_name in "${!workloads[@]}"; do
    ...
    for cache in $cache_size; do
        echo "Starting with $cache KB cache size"

        ## one-sided
        param="$onesided_config $wk CACHE_SIZE=$cache"
        run_experiment "$param"

        ## two-sided 
        param="$twosided_config $wk CACHE_SIZE=$cache"
        run_experiment "$param"
